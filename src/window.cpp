...
Window::Window() {
    ...
    background = unique_ptr<Sprite>(new Sprite("../sprites/back.png", renderer));
    background->move({0, 0});

    buttonBackground = unique_ptr<Sprite>(new Sprite("../sprites/button_back.png", renderer));

    cellBackground = unique_ptr<Sprite>(new Sprite("../sprites/cell_back.png", renderer));
    X = unique_ptr<Sprite>(new Sprite("../sprites/X.png", renderer));
    O = unique_ptr<Sprite>(new Sprite("../sprites/O.png", renderer));
...
void Window::drawMenu() {
    ...
    for(short i = 0; i < 3; i++) {
        ...
        switch(i) {
        case 0:
            textNewGame->move({buttonTopLeft.x+button.width/10,
                               buttonTopLeft.y});
            textNewGame->draw();
            break;
        case 1:
            textOptions->move({buttonTopLeft.x+button.width/6,
                               buttonTopLeft.y});
            textOptions->draw();
            break;
        case 2:
            textExit->move({buttonTopLeft.x+button.width/4,
                            buttonTopLeft.y});
            textExit->draw();
            break;
        }
        ...
        case SDL_MOUSEBUTTONDOWN:{
            ...
            if(state == State::Game) {
                ...
                for(short c = 0; c < 3; c++) {
                    ...
                    Rect buttonRect({offsetToTopLeftCorner,
                                     offsetToTopLeftCorner +
                                     (buttonSize.height+10)*c},
                                    buttonSize);
...
void Window::proccessPlayerTurn(const Point& mouse_coords) {
    static Rect field_rect({(size.width-fieldSize)/4,
                            (size.height-fieldSize)/2},
                           {fieldSize, fieldSize});

    if(field_rect.contain(mouse_coords)) {
        try {
            field.selectCell({(mouse_coords.x-(size.width-fieldSize)/4)/(fieldSize/3),
...
void Window::resize(int width, int height) {
    ...
    offsetToTopLeftCorner = (size.width > size.height)
                             ? size.height/5
                             : size.width/5;

    fieldSize = (size.width > size.height) ? size.height-offsetToTopLeftCorner*2
